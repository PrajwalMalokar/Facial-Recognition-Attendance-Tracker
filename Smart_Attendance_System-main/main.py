...
class AttendanceSystem:
    def __init__(self, window):
        ...
        self.detector = cv2.CascadeClassifier("D:\\Projects\\Smart_Attendance_System-main\\Smart_Attendance_System-main\\haarcascade_frontalface_default.xml")
        self.cap = None
    ...
    def capture_images(self):
        ...
        # Create the CSV file with the correct columns if it doesn't exist
        if not os.path.exists(csv_file_path):
            with open(csv_file_path, mode="w", newline="") as file:
                writer = csv.writer(file)
                writer.writerow(["Enrollment", "Name", "Timestamp"])
    ...
    def train_img(self):
        if not os.path.exists("Training_Image_Label"):
            os.makedirs("Training_Image_Label")
